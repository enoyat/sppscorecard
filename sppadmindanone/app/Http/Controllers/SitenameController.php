...
class SitenameController extends Controller
{
    ...
    public function getsitename(Request $request){
        if(Auth::user()->roles_id == '4' ){
            $sitename = MSitename::member(Session::get('kdcustomer'))->kategori("sitename")->where('namasitename',$request->search)->get();
        }
        else{
         
            $sitename = MSitename::member(Session::get('kdcustomer'))->kategori("sitename")->where('namasitename', 'LIKE', '%'.$request->search.'%')->orderBy('namasitename', 'ASC')->get();
        }
